...
(define refine1 : (Parameterof (.σ .V .V → .Vns))
  (make-parameter
   (λ (σ V C)
     (error 'Internal "Unimplemented `refine1`"))))

(: δ : .σ .o (Listof .V) Mon-Party → .Ans*)
(define (δ σ o V* l)
  (when (box-operation? o)
    (error 'Internal "δ should not handle box operations"))
  ...
  (match* (o V*)
    ...
    [('string-length (list V))
     (match/Ans* (δ σ 'string? V* 'Λ)
                 ...
                 [(cons σf (.// (.b #f) _)) (cons σf (.blm l 'string-length V STR/C))])]
       ...
       [(.// '• Cs)
        (or
         (for/or : (U #f .Vns)
                 ([C Cs]
                  #:when
                  (match? C (.// (.λ↓ (.λ 1 (.@ 'string-length (list (.x 0) (or (? .b?) (? .x?))) _)) ρ) _)))
           (match-define (.// (.λ↓ (.λ 1 (.@ _ (list _ len) _)) ρ) _) C)
           (cons σ (match len
                     [(? .b? b) (→V b)]
                     [(.x sd) (ρ@ ρ (- sd 1))])))
