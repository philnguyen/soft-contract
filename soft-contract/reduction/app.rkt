...
(define-unit app@
  ...
  (define (app ℓ Wₕ Wₓs $ Γ H Σ ⟦k⟧)
    ...
    (define-syntax-rule (with-guarded-arity a* e ...)
      (let ([n (length Wₓs)]
        ...
        (cond
          ...
          [else (⟦k⟧ (blm-arity a n) $ Γ H Σ)])))

    (define (app-And/C [W₁ : -W¹] [W₂ : -W¹]) : (℘ -ς)
      (define ⟦rhs⟧ (mk-app ℓ (mk-rt W₂) (list (mk-rt (car Wₓs)))))
      (app ℓ W₁ Wₓs $ Γ H Σ (and∷ l (list ⟦rhs⟧) ⊥ρ ⟦k⟧)))

    (define (app-Or/C [W₁ : -W¹] [W₂ : -W¹]) : (℘ -ς)
      (define ⟦rhs⟧ (mk-app ℓ (mk-rt W₂) (list (mk-rt (car Wₓs)))))
      (app ℓ W₁ Wₓs $ Γ H Σ (or∷ l (list ⟦rhs⟧) ⊥ρ ⟦k⟧)))
    
    (define (app-Not/C [Wᵤ : -W¹]) : (℘ -ς)
      (app ℓ Wᵤ Wₓs $ Γ H Σ (ap∷ (list (+W¹ 'not)) '() ⊥ρ ℓ ⟦k⟧)))
    ...
    (define (app-St/C [𝒾 : -𝒾] [W-Cs : (Listof -W¹)]) : (℘ -ς)
      ...
      (match Vₓ
        [(or (-St (== 𝒾) _) (-St* (-St/C _ (== 𝒾) _) _ _))
         ...
         (app ℓ P (list Wₓ) $ Γ H Σ (and∷ l ⟦chk-field⟧s ⊥ρ ⟦k⟧))]
        [_
         (⟦k⟧ (+W (list -ff)) $ Γ H Σ)]))
    ...
    (match Vₕ
      ...
      [(-st-p  𝒾) ((app-st-p 𝒾) ℓ Wₓs $ Γ H Σ ⟦k⟧)]
      [(-st-mk 𝒾) ((app-st-mk 𝒾) ℓ Wₓs $ Γ H Σ ⟦k⟧)]
      [(-st-ac  𝒾 i) ((app-st-ac  𝒾 i) ℓ Wₓs $ Γ H Σ ⟦k⟧)]
      [(-st-mut 𝒾 i) ((app-st-mut 𝒾 i) ℓ Wₓs $ Γ H Σ ⟦k⟧)]
      ['make-sequence (app-make-sequence ℓ Wₓs $ Γ H Σ ⟦k⟧)]

      ;; Regular stuff
      [(? symbol? o) ((app-prim o) ℓ Wₓs $ Γ H Σ ⟦k⟧)]
      [(-Clo xs ⟦e⟧ ρₕ Γₕ)
       (with-guarded-arity (shape xs)
         ((app-clo xs ⟦e⟧ ρₕ Γₕ sₕ) ℓ Wₓs $ Γ H Σ ⟦k⟧))]
      [(? -Case-Clo?)
       ((app-Case-Clo Vₕ sₕ) ℓ Wₓs $ Γ H Σ ⟦k⟧)]
      [(-Ar C α ctx)
         ...
         (cond
           [(-=>? C)
            (for/union : (℘ -ς) ([Vᵤ (σ@ Σ α)] #:unless (equal? Vₕ Vᵤ))
              ((app-Ar C c Vᵤ sₕ ctx) ℓ Wₓs $ Γ H Σ ⟦k⟧))]
           [(-=>i? C)
            (for/union : (℘ -ς) ([Vᵤ (σ@ Σ α)] #:unless (equal? Vₕ Vᵤ))
                       ((app-Indy C c Vᵤ sₕ ctx) ℓ Wₓs $ Γ H Σ ⟦k⟧))]
           [(-∀/C? C)
            (for/union : (℘ -ς) ([Vᵤ (σ@ σ α)] #:unless (equal? Vₕ Vᵤ))
                       ((app-∀/C C c Vᵤ sₕ ctx) ℓ Wₓs $ Γ H Σ ⟦k⟧))]
           [else
            (for/union : (℘ -ς) ([Vᵤ (σ@ Σ α)] #:unless (equal? Vₕ Vᵤ))
              ((app-guarded-Case C c Vᵤ sₕ ctx) ℓ Wₓs $ Γ H Σ ⟦k⟧))]))]
      ...
      [(->/c (? real? r))
       (app ℓ (-W¹ '> '>) (list (car Wₓs) (-W¹ (-b r) (-b r))) $ Γ H Σ ⟦k⟧)]
      [(-≥/c (? real? r))
       (app ℓ (-W¹ '>= '>=) (list (car Wₓs) (-W¹ (-b r) (-b r))) $ Γ H Σ ⟦k⟧)]
      [(-</c (? real? r))
       (app ℓ (-W¹ '< '<) (list (car Wₓs) (-W¹ (-b r) (-b r))) $ Γ H Σ ⟦k⟧)]
      ...
      [(-≢/c b)
       ...
       #;(app ℓ 'equal? (list (car Wₓs) (-W¹ (-b b) (-b b))) $ Γ H ⟦k⟧)]
  ...
  (define (app-clo xs ⟦e⟧ ρₕ Γₕ sₕ)
    (λ (ℓ Wₓs $ Γ H Σ ⟦k⟧)
      ...
      (define-values (Hₑₑ looped?) (H+ H (-edge (cons ⟦e⟧ (⌊ρ⌋ ρₕ)) ℓ)))
      ...
      (define-values (ρ* $* bnds)
        (match xs
          [(? list? xs)
           (alloc-init-args! Σ $₀ Γ ρₕ Hₑₑ xs Wₓs looped?)]
          [(-var zs z)
           (define-values (Ws₀ Wsᵣ) (split-at Wₓs (length zs)))
           (define-values (ρ₀ $₁ bnds) (alloc-init-args! Σ $₀ Γ ρₕ Hₑₑ zs Ws₀ looped?))
           (define Vᵣ (alloc-rest-args! Σ Γ Hₑₑ ℓ Wsᵣ))
  ...
  (define ((app-Case-Clo cases tₕ) ℓ Wₓs $ Γ H Σ ⟦k⟧)
    ...
    (match ?case
      [(-Clo xs ⟦e⟧ ρₕ Γₕ)
       ((app-clo xs ⟦e⟧ ρₕ Γₕ tₕ) ℓ Wₓs $ Γ H Σ ⟦k⟧)]
      [#f
  ...
  (define ((app-guarded-Case C c Vᵤ sₕ ctx) ℓ Wₓs $ Γ H Σ ⟦k⟧)
  ...
  (: app-Ar : -=> -?t -V -?t -ctx → -⟦f⟧)
  (define ((app-Ar C c Vᵤ sₕ ctx) ℓₐ Wₓs $ Γ H Σ ⟦k⟧)
    (define σ (-Σ-σ Σ))
    ...
    (match* (αℓs cs)
      ...
      [((-var αℓs₀ αℓᵣ) (-var cs₀ cᵣ))
       ...
       (define Vᵣ (alloc-rest-args! Σ Γ H ℓᵣ Wsᵣ))
  ...
  (: app-∀/C : -∀/C -?t -V -?t -ctx → -⟦f⟧)
  (define ((app-∀/C C c Vᵤ sₕ ctx) ℓₐ Wₓs $ Γ H Σ ⟦k⟧)
    ...
    (define ρ* ; with side-effect widening store
      (for/fold ([ρ : -ρ ρ]) ([x (in-list xs)])
        (σ⊕Vs! Σ (-α->⟪α⟫ (-α.sealed x Hₑₑ)) ∅)
        (hash-set ρ x (-α->⟪α⟫ (-α.imm (-Seal/C x Hₑₑ l-seal))))))
    (define ⟦k⟧*
      (restore-ctx∷ H
        (mon.v∷ ctx (-W¹ Vᵤ sₕ)
         ...
         (match Vₛ
           ...
           [(-St* (-St/C _ (== 𝒾) γℓs) α ctx)
            ...
            (for*/union : (℘ -ς) ([Vₛ* (in-set (σ@ Σ α))]
                                  ...
                                  [C (in-set (σ@ Σ γ))])
              (push-mon (ctx-with-ℓ ctx* ℓᵢ) (-W¹ C c) Wᵥ $ Γ H Σ ⟦k⟧*))]
           [(-● _)
  ...
  (: app-make-sequence : -⟦f⟧)
  (define (app-make-sequence ℓ Ws $ Γ H Σ ⟦k⟧)
  ...
  (: app-prim : Symbol → -⟦f⟧)
  (define ((app-prim o) ℓ Ws $ Γ H Σ ⟦k⟧)
    (define ⟦f⟧ (get-prim o))
  ...
  (define (app/rest/unsafe ℓ W-func W-inits W-rest $ Γ H Σ ⟦k⟧)
    (define σ (-Σ-σ Σ))
    ...
    (define (app-prim/rest o)
      (for/union : (℘ -ς) ([V-rests (in-set (unalloc σ (-W¹-V W-rest)))] #:when V-rests)
        (app ℓ W-func (append W-inits (map V->W¹ V-rests)) $ Γ H Σ ⟦k⟧)))
        ...
        [(? list? xs)
         ...
         (for/union : (℘ -ς) ([V-rests (in-set (unalloc σ (-W¹-V W-rest)))]
           ...
           ((app-clo xs ⟦e⟧ ρₕ Γₕ t-func)
            ℓ (append W-inits (map V->W¹ V-rests)) $ Γ H Σ ⟦k⟧))]
        [(-var zs z)
         (define n (length zs))
         (define num-remaining-inits (- n num-inits))
         (define-values (Hₑₑ looped?) (H+ H (-edge (cons ⟦e⟧ (⌊ρ⌋ ρₕ)) ℓ)))
         ...
         (define (app/adjusted-args! W-inits W-rest)
           (define-values (ρₕ₀ $₁ bnds) (alloc-init-args! Σ $₀ Γ ρₕ Hₑₑ zs W-inits looped?))
           (define αᵣ (-α->⟪α⟫ (-α.x z Hₑₑ ∅)))
           (σ⊕V! Σ αᵣ (-W¹-V W-rest))
           (define ρₕ* (ρ+ ρₕ₀ z αᵣ))
      ...
      (match C
        [(-=> (-var αℓs₀ (-⟪α⟫ℓ αᵣ ℓᵣ)) _)
         ...
         (cond
           ...
           [else
            (define-values (W-inits* W-inits.rest) (split-at W-inits n))
            (define-values (Hₑₑ looped?) (H+ H (-edge #|HACK|# (cons (mk-rt (-W¹ C #f)) (⌊ρ⌋ ⊥ρ)) ℓ)))
            (define V-rest* (alloc-rest-args! Σ Γ Hₑₑ ℓ W-inits.rest #:end (-W¹-V W-rest)))
            (define W-rest* (-W¹ V-rest* #f))
            (for/union : (℘ -ς) ([Vᵤ (in-set (σ@ Σ α))])
              ((apply-app-Ar C #f Vᵤ t-func ctx) ℓ W-inits* W-rest* Γ H Σ ⟦k⟧))])]
         ...
         (cond
           [(>= num-remaining-args 0)
            (for*/union : (℘ -ς) ([Vᵤ (in-set (σ@ Σ α))]
              ...
              (define W-inits* (append W-inits (map V->W¹ V-inits-more)))
              ((app-Ar C #f Vᵤ t-func ctx) ℓ W-inits* $ Γ H Σ ⟦k⟧))]
           [else
