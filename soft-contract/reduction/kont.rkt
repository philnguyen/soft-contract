...
(define-unit kont@
  ...
  (define-frame (mon.c∷ [l³ : -l³]
    ...
    (make-frame (⟦k⟧ A $ Γ ⟪ℋ⟫ Σ) #:roots (root)
      ...
      (match Vs
        [(list V)
         (define W-V (-W¹ V s))
         (cond [(-W¹? C) {set (push-mon l³ ℓ C W-V $ Γ ⟪ℋ⟫ Σ ⟦k⟧)}]
               [else
  ...
  (define-frame (mon.v∷ [l³ : -l³]
    ...
    (make-frame (⟦k⟧ A $ Γ ⟪ℋ⟫ Σ) #:roots (root)
      ...
      (match Vs
        [(list C)
         (define W-C (-W¹ C s))
         (cond [(-W¹? V) {set (push-mon l³ ℓ W-C V $ Γ ⟪ℋ⟫ Σ ⟦k⟧)}]
               [else
            ...
            (for/union : (℘ -ς) ([Ds (in-set (σ@/list Σ βs))])
              ...
              (match* (Ctcs Vals ℓs)
                [((cons Ctc₁ Ctcs*) (cons Val₁ Vals*) (cons ℓ₁ ℓs*))
                 {set (push-mon l³ ℓ₁ Ctc₁ Val₁ $ Γ ⟪ℋ⟫ Σ
    ...
    (make-frame (⟦k⟧ A $ Γ ⟪ℋ⟫ Σ) #:roots (W-Cs W-Vs)
      ...
      (match* (W-Cs W-Vs ℓs)
        [((cons W-C₁ W-Cs*) (cons W-V₁ W-Vs*) (cons ℓ₁ ℓs*))
         {set (push-mon l³ ℓ₁ W-C₁ W-V₁ $ Γ ⟪ℋ⟫ Σ
  ...
  (make-frame (⟦k⟧ A $ Γ ⟪ℋ⟫ Σ) #:roots (Wₗ Wᵣ W-V)
    ...
    (match Vs
      [(list (-b #f))
       {set (push-mon l³ ℓ Wᵣ W-V $ Γ ⟪ℋ⟫ Σ ⟦k⟧)}]
      [(list (-b #t) V)
    ...
    (make-frame (⟦k⟧ A $ Γ ⟪ℋ⟫ Σ) #:roots (W-C₁ W-C₂)
      ...
      (match Vs
        ...
        [(list (-b #t) V)
         ...
         {set (push-fc l ℓ W-C₂ (-W¹ (V+ (-Σ-σ Σ) V C₁) sᵥ) $ Γ ⟪ℋ⟫ Σ ⟦k⟧)}])))
  ...
  (define-frame (fc-or/c∷ [l : -l]
    ...
    (make-frame (⟦k⟧ A $ Γ ⟪ℋ⟫ Σ) #:roots (W-C₁ W-C₂)
      ...
      (match Vs
        [(list (-b #f))
         {set (push-fc l ℓ W-C₂ W-V $ Γ ⟪ℋ⟫ Σ ⟦k⟧)}]
        [(list (-b #t) V)
         (match-define (-W¹ C₁ _) W-C₁)
  ...
  (define-frame (fc.c∷ [l : -l]
    ...
    (make-frame (⟦k⟧ A $ Γ ⟪ℋ⟫ Σ) #:roots (W-C)
      ...
      (match Vs
        [(list V)
         {set (push-fc l ℓ W-C (-W¹ V s) $ Γ ⟪ℋ⟫ Σ ⟦k⟧)}]
        [_
         (define blm (-blm l 'Λ '(|1 value|) Vs ℓ))
         (⟦k⟧ blm $ Γ ⟪ℋ⟫ Σ)])))
