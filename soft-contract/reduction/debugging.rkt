...
(define-unit debugging@
  ...
  (: print-stat : (℘ -ς) → Void)
  (define (print-stat ςs)
    (define m : (HashTable -αₖ (℘ (List -$ -Γ -A))) (make-hash))
    (for ([ς ςs] #:when (-ς↓? ς))
      (match-define (-ς↓ αₖ $ Γ A) ς)
      (hash-update! m αₖ (λ ([ans : (℘ (List -$ -Γ -A))]) (set-add ans (list $ Γ A))) mk-∅))
    (define n (count-max m))
    (for ([(αₖ ans) (in-hash m)] #:when (equal? n (set-count ans)))
      (printf "- ~a -> ~a~n" (show-αₖ αₖ) (set-count ans))
      (define-set $s : -$)
      (define-set Γs : -Γ)
      (define-set As : -A)
      (for ([a : (List -$ -Γ -A) (in-set ans)])
        (match-define (list $ Γ A) a)
        ($s-add! $)
        (Γs-add! Γ)
        (As-add! A))
      (printf "  --> ~a caches, ~a pcs, ~a answers~n" (set-count $s) (set-count Γs) (set-count As))
      (printf "Answers:~n")
      (for ([A (in-set As)])
        (printf "- ~a~n" (show-A A)))
      (printf "Path-conditions:~n")
      (let-values ([(Γ₀ Γs) (factorize-pc Γs)])
        (for ([Γ (in-set Γs)])
          (cond [(set-empty? Γ) (printf "- ∅~n")]
                [else (printf "- ~a~n" (show-t (set-first Γ)))
                      (for ([t (in-set (set-rest Γ))])
                        (printf "- ~a~n" (show-t t)))])))
      ))

  (: factorize-pc : (℘ -Γ) → (Values -Γ (℘ -Γ)))
  (define (factorize-pc Γs)
    (cond
      [(set-empty? Γs) (error 'factorize-pc "empty path-condition set")]
      [else
       (define Γ₀
         (for/fold ([acc : -Γ (set-first Γs)])
                   ([Γ (in-set (set-rest Γs))])
           (∩ acc Γ)))
       (define Γs* (for/set: : (℘ -Γ) ([Γ (in-set Γs)])
                     (set-subtract Γ Γ₀)))
       (values Γ₀ Γs*)]))
