...
(define ((app-clo xs ⟦e⟧ ρₕ Γₕ sₕ) $ ℒ Wₓs Γ ⟪ℋ⟫ Σ ⟦k⟧)
  ...
  (cond
    [plausible?
     ...
     (define αₖ (-ℬ xs ⟦e⟧ ρ*))
     (define κ (-κ (make-memoized-⟦k⟧ ⟦k⟧) (-Γ-facts Γ) ⟪ℋ⟫ sₓs))
     (σₖ⊕! Σ αₖ κ)
     {set (-ς↑ αₖ Γₕ* ⟪ℋ⟫ₑₑ)}]
    [else ∅]))
...
(define ((apply-app-clo xs ⟦e⟧ ρₕ Γₕ sₕ) $ ℒ Ws₀ Wᵣ Γ ⟪ℋ⟫ Σ ⟦k⟧)
  ...
  (define κ
    (let ([ss₀ (map -W¹-t Ws₀)]
          [sᵣ (-W¹-t Wᵣ)])
      (-κ (make-memoized-⟦k⟧ ⟦k⟧) (-Γ-facts Γ) ⟪ℋ⟫ `(,sₕ ,@ss₀ ,sᵣ))))
  (σₖ⊕! Σ αₖ κ)
  {set (-ς↑ αₖ Γₕ ⟪ℋ⟫ₑₑ)})
...
(define ((app-opq sₕ) $ ℒ Ws Γ ⟪ℋ⟫ Σ ⟦k⟧)
  ...
  (for ([W (in-list Ws)])
    (add-leak! Σ (-W¹-V W)))
  (define αₖ (-ℋ𝒱))
  (define κ (-κ (bgn0.e∷ (-W -●.Vs sₐ) '() ⊥ρ ⟦k⟧) (-Γ-facts Γ) ⟪ℋ⟫ '()))
  (σₖ⊕! Σ αₖ κ)
  {set (-ς↑ αₖ ⊤Γ ⟪ℋ⟫∅)})
...
(define (mon-x/c l³ $ ℒ W-C W-V Γ ⟪ℋ⟫ Σ ⟦k⟧)
  ...
  (for/set: : (℘ -ς) ([C* (σ@ Σ ⟪α⟫)])
    ...
    (define κ (-κ ⟦k⟧ (-Γ-facts Γ) ⟪ℋ⟫ (list v)))
    (σ⊕! Σ Γ ⟪α⟫ᵥ W-V)
    (σₖ⊕! Σ αₖ κ)
    (-ς↑ αₖ ⊤Γ ⟪ℋ⟫ₑₑ)))
