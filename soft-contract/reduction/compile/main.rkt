  ...
  (define (↓d d)
    (match d
      [(-define-values xs e)
       ...
       (define ⟦e⟧ (↓ₑ l e))
       (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
         (⟦e⟧ ρ Γ 𝒞 σ σₖ M (def∷ l αs ⟦k⟧)))]
      [(-provide specs)
       (match (map ↓pc specs)
         ['() ⟦void⟧]
         [(cons ⟦spec⟧ ⟦spec⟧s)
          (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
            (⟦spec⟧ ρ Γ 𝒞 σ σₖ M (bgn∷ ⟦spec⟧s ρ ⟦k⟧)))])]
      [(? -e? e) (↓ₑ l e)]
      [_
   ...
   (match e
     [(-λ xs e*)
      ...
      (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
        ...
        (⟦k⟧ (-W (list (-Clo xs ⟦e*⟧ ρ Γ)) s) Γ 𝒞 σ σₖ M))]
     [(-case-λ clauses)
      ...
      (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
        (define s (canonicalize-e Γ e))
        (⟦k⟧ (-W (list (-Case-Clo ⟦clause⟧s ρ Γ)) s) Γ 𝒞 σ σₖ M))]
     [(? -prim? p) (↓ₚᵣₘ p)]
     [(-• i)
      (define W (-W -●/Vs e))
      (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
      ...
      (cond
        ...
        [else
         ...
         (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
           ...
           (for/union : (℘ -ς) ([V Vs])
             (define s (or (V->s σ V) ?𝒾))
             (⟦k⟧ (-W (list (supply-negative-party l V)) s) Γ 𝒞 σ σₖ M)))])]
     [(-@ f xs ℓ)
      (define ⟦f⟧  (↓ f))
      (define ⟦x⟧s (map ↓ xs))
      (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
        (⟦f⟧ ρ Γ 𝒞 σ σₖ M (ap∷ '() ⟦x⟧s ρ l ℓ ⟦k⟧)))]
     [(-if e₀ e₁ e₂)
      (define ⟦e₀⟧ (↓ e₀))
      (define ⟦e₁⟧ (↓ e₁))
      (define ⟦e₂⟧ (↓ e₂))
      (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
        (⟦e₀⟧ ρ Γ 𝒞 σ σₖ M (if∷ l ⟦e₁⟧ ⟦e₂⟧ ρ ⟦k⟧)))]
     ...
     [(-begin0 e₀ es)
      ...
      (define ⟦e⟧s (map ↓ es))
      (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
        (⟦e₀⟧ ρ Γ 𝒞 σ σₖ M (bgn0.v∷ ⟦e⟧s ρ ⟦k⟧)))]
     [(-quote q)
      (cond
        [(Base? q)
         (define b (-b q))
         (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
           (⟦k⟧ (-W (list b) b) Γ 𝒞 σ σₖ M))]
     ...
     [(-let-values bnds e*)
      ...
      (match ⟦bnd⟧s
        ...
        [(cons (cons xs ⟦e⟧ₓₛ) ⟦bnd⟧s*)
         (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
           (⟦e⟧ₓₛ ρ Γ 𝒞 σ σₖ M (let∷ l xs ⟦bnd⟧s* '() ⟦e*⟧ ρ (rst∷ (dom ρ #:eq? #t) ⟦k⟧))))])]
     ...
     [(-set! x e*)
      (define ⟦e*⟧ (↓ e*))
      (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
        (⟦e*⟧ ρ Γ 𝒞 σ σₖ M (set!∷ (ρ@ ρ x) ⟦k⟧)))]
     [(-error msg)
      (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
        (⟦k⟧ (-blm l 'Λ '() (list (-b msg))) Γ 𝒞 σ σₖ M))]
     [(-amb es)
      (define ⟦e⟧s (set-map es ↓))
      (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
        (for/union : (℘ -ς) ([⟦e⟧ ⟦e⟧s]) (⟦e⟧ ρ Γ 𝒞 σ σₖ M ⟦k⟧)))]
     [(-μ/c x c)
      (define ⟦c⟧ (↓ c))
      (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
        (⟦c⟧ ρ Γ 𝒞 σ σₖ M (μ/c∷ l x ⟦k⟧)))]
     [(--> cs d ℓ)
      (define ⟦d⟧  (↓ d))
      (match (map ↓ cs)
        ['()
         (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
           (⟦d⟧ ρ Γ 𝒞 σ σₖ M (-->.rng∷ l '() ℓ ⟦k⟧)))]
        [(cons ⟦c⟧ ⟦c⟧s)
         (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
           (⟦c⟧ ρ Γ 𝒞 σ σₖ M (-->.dom∷ l '() ⟦c⟧s ⟦d⟧ ρ ℓ ⟦k⟧)))])]
     [(-->i cs (and mk-d (-λ xs d)) ℓ)
      ...
      (match (map ↓ cs)
        ...
        [(cons ⟦c⟧ ⟦c⟧s)
         (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
           (define Mk-D (-Clo xs ⟦d⟧ ρ Γ))
           (⟦c⟧ ρ Γ 𝒞 σ σₖ M (-->i∷ '() ⟦c⟧s ρ Mk-D mk-d ℓ ⟦k⟧)))])]
     [(-case-> clauses ℓ)
      ...
      (match ⟦clause⟧s
        ['()
         (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
           (⟦k⟧ (-W (list (-Case-> '() ℓ)) e) Γ 𝒞 σ σₖ M))]
        [(cons (cons ⟦c⟧ ⟦c⟧s) ⟦clause⟧s*)
         (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
           (⟦c⟧ ρ Γ 𝒞 σ σₖ M (case->∷ l ℓ '() '() ⟦c⟧s ⟦clause⟧s* ρ ⟦k⟧)))])]
     [(-x/c x)
      (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
        (⟦k⟧ (-W (list (-x/C (-α.x/c x))) e) Γ 𝒞 σ σₖ M))]
     [(-struct/c si cs ℓ)
      (match (map ↓ cs)
        ['()
         (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
           (⟦k⟧ (-W (list (-St/C #t si '())) e) Γ 𝒞 σ σₖ M))]
        [(cons ⟦c⟧ ⟦c⟧s)
         (λ (ρ Γ 𝒞 σ σₖ M ⟦k⟧)
           (⟦c⟧ ρ Γ 𝒞 σ σₖ M (struct/c∷ ℓ si '() ⟦c⟧s ρ ⟦k⟧)))])]
