...
(define (-sexp-and [es : (Listof Sexp)]) : Sexp
  (match (filter-not tt? es)
    ['() #t]
    [(list _ ... (? ff?) _ ...) #f]
    [(list e) e]
    [es `(and ,@es)]))
(define (-sexp-or [es : (Listof Sexp)]) : Sexp
...
(define (mk-cond [x : Symbol] [c : Sexp]) : Sexp
  (let go : Sexp ([c : Sexp c])
    (match c
      ...
      [`(and/c ,ps ...) (-sexp-and (map go (cast ps (Listof Sexp))))]
      [`(or/c ,ps ...) (-sexp-or (map go (cast ps (Listof Sexp))))])))
