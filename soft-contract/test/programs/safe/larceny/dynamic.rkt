...
(struct elem ([parent #:mutable] [rank #:mutable]) #:transparent)
(struct rank ([n #:mutable] [info #:mutable]) #:transparent)
...
(define (find! elem)
  ...
  ;(newline)
  (let ((p-elem (elem-parent elem)))
    (if (null? p-elem)
	elem
	(let ((rep-elem (find! p-elem)))
	  (set-elem-parent! elem rep-elem)
	  rep-elem))))
...
(define (link! elem-1 elem-2)
  ...
  ;(newline)
  (let ((rank-1 (rank-n (elem-rank elem-1)))
	(rank-2 (rank-n (elem-rank elem-2))))
    (cond
     ((= rank-1 rank-2)
      (set-rank-n! (elem-rank elem-2) (+ rank-2 1))
      (set-elem-parent! elem-1 elem-2)
      elem-2)
     ((> rank-1 rank-2)
      (set-elem-parent! elem-2 elem-1)
      elem-1)
     (else
      (set-elem-parent! elem-1 elem-2)
      elem-2))))

(define asymm-link! (lambda (l x) (set-elem-parent! l x)))
...
(struct type (id [def #:mutable]) #:transparent)
...
(define (gen-tvar)
  ...
  (gen-element (type (gen-id) '())))

(define (gen-type tcon targs)
  ; generates a new type variable with an associated type definition
  (gen-element (type (gen-id) (cons tcon targs))))

(define dynamic (gen-element (type 0 '())))
...
(define io-env
  (list
   ...
   (cons 'display (forall (lambda (tv)
			    (procedure (convert-tvars (list tv)) dynamic))))
   (cons 'newline (procedure (fake-null) dynamic))
...
(define *forms* (file->list "dynamic.sch"))
