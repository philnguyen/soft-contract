...
(provide (struct-out stream)
         (contract-out [make-stream (-> any/c (-> stream?) stream/c)]
                       [stream-unfold (-> stream? (values any/c stream/c))]
                       [stream-get (-> stream? integer? any/c)]
                       [stream-take (-> stream? integer? list?)]
                       [leak stream?]))
