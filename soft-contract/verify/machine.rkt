...
(define (ev p)
  ...
  (: Ξ@ : .rt/κ → (Sequenceof .K)) ; FIXME force randomness to test
  (define (Ξ@ ctx) (hash-ref Ξ ctx (λ () ∅)))

  (: M@ : .rt/κ → (Sequenceof .res)) ; FIXME force randomness to test
      ...
      (when (for/and : Boolean ([(i j) (in-hash F)])
        ...
        (define-values (σₖ* F*)
          (for/fold ([σₖ : .σ σₖ] [F : .F F]) ([i (in-hash-keys F)])
            (define-values (σₖ* _ F*) (transfer σ₀ (.L i) σₖ F))
            ;(printf "Intermmediate:~n  σₖ*: ~a~n  F*: ~a~n" (show-σ σₖ*) (show-F F*))
            (values σₖ* F*)))
        
        (define-values (σₖ** V-new _) (transfer σ₀ V₀ σₖ* F*))
    ...
    (define (visit ς)
      (match ς
        ...
        [(.ς (-Vs V) σ (cons (.blr/κ F1 σ1 V1) (cons (.blr/κ F0 σ0 V0) k)))
         ...
         (define-values (σ* Vi) (⊕ σ0 V0 σ1 V1))
         (define σi (⊕/σ σ0 σ* F0))
...
(define (step-p ms)
         ...
         [(= (length Vx) n)
          ...
          (or
           ...
           (for/or : (U #f .ς) ([res : (Pairof .rt/κ (Option .F)) seens]
             ...
             (define-values (σ1 Vx1) (⊕* σ0 Vx0 σ Vx))
      ...
      [(.// U C*)
       (match U
         ...
         [(.Ar (.// (.Λ/C C* D v?) _) Vg (and l³ (list _ _ lo)))
          ...
          (define C# (length C*))
          (define n (if v? (- C# 1) #f))
          (define arity-ok? (if v? (>= V# (- C# 1)) (= V# C#)))
          (if arity-ok?
            ...
            [(and (.μ/C x C′) U)
             (cond
               [(chk-seen? k U (V-abs σ V))
                (define-values (σ* _) (refine σ V C))
                (.ς -VsTT σ* k)]
               [else
            ...
            [(and (.μ/C x C′) Uc)
             (cond
               ...
               [(flat/C? σ C)
                ...
                (define-values (σt Vt) (refine σ V C))
                (define-values (σf _) (refine σ V (.¬/C C)))
  ...
  (define (step-e e ρ σ k)
    (match e
      ...
      [(.letrec-values '() e ctx) (step-e e ρ σ k)]
      [(.letrec-values (and bnds (cons (cons nₓ eₓ) bnds*)) e ctx)
       (define ρ* (ρ-undefined ρ (for/sum ([bnd bnds]) (car bnd))))
       (.ς (.↓ eₓ ρ*) σ (cons (.letrec-values/κ nₓ bnds* ρ* e ctx) k))]
  ...
  (define (step-Vs Vs σ κ k)
    (match κ
      ...
      [(.letrec-values/κ n bnds ρ e ctx)
       (cond
         [(= n (length Vs))
          (parameterize ([abstract-V? #f])
            (printf "ρ₀: ~a~n" (show-ρ σ ρ)))
          (match bnds
            [(list)
             (parameterize ([abstract-V? #f])
               (printf "ρ₀′: ~a~n" (show-ρ σ (ρ-upd ρ 0 Vs))))
             (.ς (.↓ e (ρ-upd ρ 0 Vs)) σ k)]
            [(cons (cons nₓ eₓ) bnds*)
             (define ρ* (ρ-upd ρ (for/sum ([bnd bnds]) (car bnd)) Vs))
             (parameterize ([abstract-V? #f])
               (printf "ρ₁: ~a~n" (show-ρ σ ρ)))
             (.ς (.↓ eₓ ρ*) σ
                 (cons (.letrec-values/κ nₓ bnds* ρ* e ctx) k))])]
         [else (.ς (.blm ctx 'letrec-values (Prim (length Vs)) (=/C (Prim n))) σ k)])]
      [κ (match Vs
...
(define (canon ς)
  ...
  (define (go! x)
    (match x
      ...
      [(.letrec-values/κ n bnds ρ e ctx)
       (.letrec-values/κ n bnds (go! ρ) e ctx)]
  ...
  (define fixup/E
    (match-lambda
     ...
     [(.Vs Vs) (.Vs (map fixup/V Vs))]))

  (: fixup/U : (case-> [.μ/C → .μ/C] [.λ↓ → .λ↓] [.U → .U]))
  (define fixup/U
    (match-lambda
     [(.Ar c v l) (.Ar (fixup/V c) (fixup/V v) l)]
     [(.St t V*) (.St t (map fixup/V V*))]
     [(.λ↓ f ρ) (.λ↓ f (fixup/ρ ρ))]
     [(.Λ/C c d v?) (.Λ/C (map fixup/V c) (fixup/E d) v?)]
     [(.St/C t V*) (.St/C t (map fixup/V V*))]
  ...
  (define fixup/κ
    (match-lambda
     ...
     [(.let-values/κ n bnds vals ρ e ctx)
      (.let-values/κ n bnds (map fixup/V vals) (fixup/ρ ρ) e ctx)]
     [(.letrec-values/κ n bnds ρ e ctx)
      (.letrec-values/κ n bnds (fixup/ρ ρ) e ctx)]
     [(.@/κ e* v* l) (.@/κ (map fixup/E e*) (map fixup/V v*) l)]
     [(.▹/κ (cons C E) l)
      (.▹/κ (cond [(and (false? C) (.E? E)) (cons #f (fixup/E E))]
            ...
            l)]
     [(.indy/κ c x x↓ d v? l)
      (.indy/κ (map fixup/V c) (map fixup/V x) (map fixup/V x↓) (if (.↓? d) (fixup/E d) #f) v? l)]
     [(? .μc/κ? x) x]
     [(.λc/κ c c↓ d ρ v?) (.λc/κ c (map fixup/V c↓) d (fixup/ρ ρ) v?)]
     [(.structc/κ t c ρ c↓) (.structc/κ t c (fixup/ρ ρ) (map fixup/V c↓))]
     #;[(.rt/κ σ f x) (.rt/κ (fixup σ) (fixup f) (fixup x))]
     #;[(.blr/κ G σ V) (.blr/κ G (fixup σ) (fixup V))]
     #;[(.recchk/κ C V) (.recchk/κ (fixup C) (fixup V))]
     [(.μ/κ f xs σ) (.μ/κ f (map fixup/V xs) (fixup/σ σ))]))

  (: fixup/σ : .σ → .σ)
  (define (fixup/σ σ)
    (match-define (.σ m _) σ)
    (define-values (σ* _) (σ++ σ∅ (hash-count F)))
    (for/fold ([σ* : .σ σ*]) ([(i i*) (in-hash F)])
      (cond
        [(hash-ref m i #f) =>
         (λ (Vᵢ) (σ-set σ* i* (subst/L Vᵢ F)))]
        [else σ*])))

  (define E′ (go! E))
  (define k′ (go! k))
  (.ς (fixup/E E′) (fixup/σ σ) (map fixup/κ k′)))
