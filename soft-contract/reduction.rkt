...
(define (↦ p)
    ...
    (match e
      ...
      [(-let-values bnds e* l)
       (match bnds
         ...
         [(cons (cons xs eₓ) bnds*)
          (define τ* (τ↓ eₓ ρ Γ))
          (define κ* (-κ (-φ.let-values xs bnds* (hash) ρ e l) τ))
          (define Ξ* (⊔ Ξ τ* κ*))
          (-ς (-↓ eₓ ρ) Γ τ* σ Ξ* M)])]
      ;; letrec-values
      [(-letrec-values bnds e l)
       (match bnds
         ...
         [(cons (cons xs e*) bnds*)
          (define-values (ρ* σ*)
            (for*/fold ([ρ* : -ρ ρ] [σ* : -σ σ]) ([bnd bnds] [x (car bnd)])
              (define α (alloc e x Γ)) ; monovariant for now
              (values (ρ+ ρ* x α) (⊔ σ α 'undefined))))
          (define τ* (τ↓ e* ρ* Γ))
          (define κ* (-κ (-φ.letrec-values xs bnds* ρ* e l (dom ρ)) τ))
          (define Ξ* (⊔ Ξ τ* κ*))
          (-ς (-↓ e* ρ*) Γ τ* σ* Ξ* M)])]
      ;; @-havoc
      [(-@-havoc x)
      ...
      [(--> cs d)
       (match cs
         ...
         [(cons c cs*)
          (define τ* (τ↓ c ρ Γ))
          (define κ* (-κ (-φ.=> (append cs* (list d)) '() ρ) τ))
          (define Ξ* (⊔ Ξ τ* κ*))
          (-ς (-↓ c ρ) Γ τ* σ Ξ* M)])]
      [(-->i doms rng)
       (match doms
         ...
         [(cons dom doms*)
          ...
          (define τ* (τ↓ c ρ Γ))
          (define κ* (-κ (-φ.=>i cs* '() (cons x xs*) rng ρ) τ))
          (define Ξ* (⊔ Ξ τ* κ*))
          (-ς (-↓ c ρ) Γ τ* σ Ξ* M)])]
      [(-x/c x)
       (error '↦e "TODO: x/c")]
      [(-struct/c id cs)
       (match cs
         ...
         [(cons c cs*)
          (define τ* (τ↓ c ρ Γ))
          (define κ* (-κ (-φ.struct/c id cs* ρ '()) τ))
          (define Ξ* (⊔ Ξ τ* κ*))
          (-ς (-↓ c ρ) Γ τ* σ Ξ* M)])]
      ))
  ...
  (match-lambda
   ...
   [(-ς (and W (-W (? -blm?) _)) Γ τ σ Ξ M) (-ς W Γ τ₀ σ Ξ M)]
   [(-ς (and W (-W 'undefined π)) Γ τ σ Ξ M)
    (cond
      [(defines? σ Γ π) ∅] ; spurious case
      [else (-ς W Γ τ₀ σ Ξ M)])]
   [(-ς (? list? Ws) Γ τ σ Ξ M)
...
(define (alloc-bnd [ctx : -e] [Γ : -Γ] [sd : Integer])
  (alloc ctx Γ sd))
