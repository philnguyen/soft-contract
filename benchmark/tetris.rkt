...
(module unsafe-consts racket
  (define block-size 20)
  (define board-height 20)
  (define board-width 10)
  (provide block-size board-height board-width))
...
(module unsafe-image racket
  ...
  (require (submod ".." unsafe-data))
  (define image/c image?)
  (provide image/c overlay circle rectangle place-image empty-scene))
...
(module unsafe-list-fun racket  
  (require (submod ".." unsafe-image)
           (submod ".." unsafe-data))
  
  (define foldr-i foldr)
  (define foldr-n foldr)
  (provide max min ormap andmap map filter append length foldr foldr-i foldr-n))
...
(module unsafe-bset racket
  (require (submod ".." unsafe-data)
           (submod ".." unsafe-block)
           (submod ".." unsafe-list-fun)
  ...
  (provide
   blocks-contains? blocks=? blocks-subset? blocks-intersect
   blocks-count blocks-overflow? blocks-move blocks-rotate-cw blocks-rotate-ccw
...
(module unsafe-tetras racket  
  (require (submod ".." unsafe-bset)
           (submod ".." unsafe-data)
           (submod ".." unsafe-consts)
           (submod ".." unsafe-block))
  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  (provide
   tetra-move tetra-rotate-ccw tetra-rotate-cw
   tetra-overlaps-blocks? build-tetra-blocks tetra-change-color))
...
(module unsafe-aux racket
  ...
  (provide list-pick-random neg-1 tetras))
...
(module unsafe-visual racket    
  ...
  (provide world->image blocks->image block->image place-block world0))
...
(require 'visual 'world 'bset 'data)
(require 2htdp/universe)
(require (prefix-in unsafe: 'unsafe-data)
         (prefix-in unsafe: 'unsafe-consts)
         (prefix-in unsafe: 'unsafe-block)
         ...
         (prefix-in unsafe: 'unsafe-world)
         (prefix-in unsafe: 'unsafe-visual))
